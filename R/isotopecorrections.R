...
correct_interferences <- function(data, variable = "feature_intensity") {
  ...
  d_corrected_features <- data@dataset |>
    mutate(
      is_interfering = .data$feature_name %in% data@annot_features$interference_feature_name,
    ...
    mutate(
      value_corr_interf = if_else(!.data$is_interfering & !is.na(.data$interference_feature_name), .data$feature_intensity_raw - .data$feature_intensity_raw[.data$is_interfering] * .data$interference_proportion, NA_real_),
  ...
  n_corr <- data@annot_features |>
    filter(!is.na(.data$interference_feature_name)) |>
    nrow()
