...
normalize_by_istd <- function(data, error_missing_info = TRUE) {
  ...
  istds <- data@annot_features |> filter(.data$is_istd) |> pull(feature_id)
  n_used_istds <- length(intersect(istds, data@annot_features |> filter(!.data$is_istd) |> pull(quant_istd_feature_id) |> unique()))
...
#' @return MidarExperiment object
#' @export
quantitate_by_istd <- function(data, error_missing_info = TRUE, ignore_unused_istds = TRUE) {
  ...
  istds <- data@annot_features |> filter(.data$is_istd) |> pull(feature_id)
  unused_istds <- setdiff(istds, data@annot_features |> filter(!.data$is_istd) |> pull(quant_istd_feature_id) |> unique())
