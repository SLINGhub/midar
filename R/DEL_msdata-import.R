...
#'   data <- rawdata_import_main(data = data, path = path, import_function = "parse_mrmkit_result", file_ext = "*.tsv|*.csv", use_metadata = use_metadata, silent = FALSE)
#'   if (use_metdadata) data <- metadata_from_data(data, analysis_sequence = "resultfile", qc_type_field = "sample_type")
#'   data
#' }
#'
#'
...
#'
#'
#' rawdata_import_main <- function(data, path, import_function, file_ext, silent, ...) {
#'
#'   if (!fs::is_dir(path)) {
...
#' #' @export
#' #'
#' parse_analysisresult_table <- function(file, value_type = c("area", "height", "intensity", "norm_area", "norm_height", "norm_intensity", "feature_conc", "rt", "fwhm", "width"), sheet = "", silent = FALSE) {
