...
plot_rt_vs_chain <- function(data = NULL,
  ...
  if(inherits(data, "MidarExperiment")) {
    ...
    cli::cli_abort("The input data must be a `MidarExperiment` object or a `data.frame`.")
  }
  ...
  d_lipid_info <- data@dataset |>
    dplyr::select("analysis_id", "feature_id", "qc_type", "is_istd", "is_quantifier", "feature_class", "feature_rt") |>
  ...
  if (x_var == "ecn") {
    d_lipid_info <- d_lipid_info |>
      dplyr::mutate(ecn = .data$total_c - ecn_k * .data$total_db,
                    total_grp = 0,)
    x_var <- "ecn"
    group_var <- "total_grp"
    col_var <- "total_db"
    x_title <- glue::glue("Equivalent Carbon Number (ECN) of the chains (ecn_k = {ecn_k})")
  ...
  fit_models <- function(data, x_var, group_var, remove_outlier_order) {
    ...
    if (remove_outlier_order) data <- data |> mutate(rt_median = if_else(.data$is_outlier_order, NA_real_, .data$rt_median))
    ...
    fitted_lines <- data |>
      ...
      mutate(model = purrr::map(data, ~ {
        if (n_distinct(.x[[x_var]]) >= 3) {
          n <- nrow(.x)
          weights <- rep(1, n)
          if(n >= 1) weights[1] <- 0.8       # Set first value
          if(n >= 2) weights[n] <- 0.6       # Set last value (if n >= 2)
          # Robust regression with weights
          tryCatch(
            withCallingHandlers(
              if (robust_regression) {
                MASS::rlm(rt_median ~ poly(.x[[x_var]], 2, raw = FALSE), data = .x, weights = weights)
              } else {
  ...
  avg_rt_median <- d_plot |>
    ...
    mutate(
      ...
      next_avg_rt = dplyr::lead(.data$avg_rt, default = Inf)) |>  # Access average from the next group
  ...
  fitted_lines <- fit_models(d_plot, x_var, group_var, remove_outlier_order = TRUE)

  d_plot <- d_plot |>
    left_join(fitted_lines |> select("feature_id", ".fitted"), by = "feature_id", "lipid_class_lcb") |>
    group_by(.data$lipid_class_lcb, !!sym(group_var)) |>
    arrange(desc(!!sym(x_var)), .by_group = TRUE,) |>
    mutate(
      # Flag if current rt_median is less than or equal to the average rt_median of the previous x_var
  ...
  d_plot <- d_plot |>
    mutate(
      ...
      point_fill = ifelse(.data$is_outlier, as.character(!!sym(col_var)), NA_character_),
  ...
  d_plot <- d_plot |>
    mutate(
      is_outlier_plot = ifelse(!is.na(.data$is_outlier_plot) & .data$is_outlier_plot, "Yes", "No")
    )
  ...
  alt_bright_dark_colors <- c(
    "#023557",  "#40bce6",  "#018c45",  "#a1f073",
    "#E31A1C",  "#FDBF6F",  "#6a2185",  "#bfaed4",
    "#FF7F00",  "#AAAAAA",  "#8B4513"
  ...
  #browser()
  p <- ggplot(d_plot |> drop_na(!!sym(x_var)),
  ...
  if (x_var == "ecn") {
    p <- p + geom_line(
      data = fitted_lines,
      aes(x = !!sym(x_var), y = .data$.fitted), color = "#afc7c7",
      inherit.aes = FALSE,
      linewidth = 0.7,
  ...
  stroke = 0.5
    ...
    facet_wrap(vars(.data$lipid_class_lcb),
               ncol = cols_page,
               scales = "free") +
    scale_x_continuous(
      breaks = function(x) seq(floor(min(x, na.rm = TRUE) / 2) * 2,
                               ceiling(max(x, na.rm = TRUE) / 2) * 2, by = 2),expand = c(0.1, 0)
    ) +
    scale_y_continuous(
      expand = c(0.1, 0)
    )
  ...
  p <- p +scale_color_manual(values = rep(alt_bright_dark_colors, length.out = n_colors_needed)) +
    scale_fill_manual(values = rep(alt_bright_dark_colors, length.out = n_colors_needed), na.value = "white") +
    # Show open symbol for non-outlier, filled for outlier in legend
    scale_shape_manual(values = c("No" = 1, "Yes" = 8)) +
    labs(x = x_title, y = "Median Retention Time",
         shape = "Outlier") +
  ...
  if(outlier_print) {
    ...
    if (length(species) > 0) {
      cli::cli_alert_info("The following features were flagged as potential annotation outliers: {glue::glue_collapse(species, sep = ', ')}")
    } else {
