...
plot_calibcurves_page <- function(dataset,
  ...
  p <- ggplot(
      ...
      ggpmisc::stat_poly_line(
        data = subset(dat_subset, dat_subset$concentration <= max_regression_value),
        aes(
          x = .data$concentration,
          y = !!plot_var,
          color = .data$curve_id
        ),
        se = TRUE, na.rm = TRUE, size = line_width * scaling_factor, inherit.aes = FALSE
      ) +
...
plot_calibrationcurves <- function(data = NULL,
  ...
  dat_filt <- dat_filt |> dplyr::semi_join(data@annot_calibcurves, by = c("analysis_id", "feature_id"))
  ...
  d_calib <- dat_filt |>
    ...
    dplyr::right_join(data@annot_calibcurves, by = c("analysis_id" = "analysis_id", "feature_id" = "feature_id"))
