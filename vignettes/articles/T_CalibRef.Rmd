...
$$
C_\text{cal}^\text{Analyte} = \frac{C_\text{sample}^\text{Analyte}}{C_\text{ref}^\text{Analyte}} \times C_\text{known}^\text{Analyte}
$$
...
In the MiDAR XLSX report generated by `save_report_xlsx()`, the unfiltered dataset with normalized
concentrations is included by default. To include the normalized concentrations as the filtered
dataset, set `filtered_variable = “[VARIABLE]_normalized` as an argument.
...
$$
c_\text{ratio}^\text{Analyte} = \frac{c_\text{measured}^\text{Analyte}}{c_\text{expected}^\text{Analyte}}
$$
...
tbl <- get_qc_bias_variability(mexp, qc_types = "NIST")
gt::gt(tbl) |> gt::fmt_number(decimals = 3)
