...
To check for potential peak picking errors, the retention time (RT) of lipid features can be plotted against the total carbon count and double bond number. Featyures that deviate  from the trend are labelled and indicate potential misannotations or peak picking errors. However, the robustness of this approach depends on the comprehensiveness of the lipid species covered in the analysis. In this example, only a limited number of lipid featires were measured, and thus the trend is not well defined for some classes and double bond counts.
...
## 22. Sharing the `MidarExperiment` dataset
