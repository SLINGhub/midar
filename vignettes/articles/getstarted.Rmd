...
myexp <- import_data_mrmkit(data = myexp, path = data_path, include_metadata = TRUE)
```
...
myexp <- exclude_analyses(myexp, analyses_exlude = c("Longit_batch6_51"), replace_existing  = TRUE)
...
overwriting or amending (`replace_existing = FALSE`) previously set
filters.
...
```{r qc-summary-1}
midar::plot_summary_classes(myexp, include_qualifier = FALSE)
```
...
```{r qc-summary-2}
midar::plot_summary(myexp, include_qualifier = FALSE)
```
