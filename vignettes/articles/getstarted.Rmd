...
vignette: >
  %\VignetteIndexEntry{MiDAR workflow}
...
setting `use_metadata = TRUE`.
<details>
<summary>See exercises</summary>
...
data_path <- system.file("extdata", "sPerfect_MRMkit.tsv", package = "midar")
myexp <- data_import_mrmkit(data = myexp, path = data_path, use_metadata = TRUE)
```
...
```{r import-metadat}
file_path <- system.file("extdata", "Example_Metadata_1.xlsm", package = "midar")
myexp <- midar::metadata_import_midarxlm(myexp, path = file_path, ignore_warnings = TRUE)
...
> `#analysis_no_range =`. What do you see in this batch? Identify the
...
> Change the plotted lipid species by modifying `feature_incl_filt` (it
> can use regular expressions). Save a PDF of all lipids by setting
> `save_pdf = TRUE` and commenting out (add a `#` in front of)
> `feature_incl_filt`
...
qc_plot_responsecurves(
  ...
  filter_data = TRUE, 
  feature_incl_filt = "", 
  save_pdf = TRUE, path = "response-curves.pdf", 
  cols_page = 5, rows_page = 5
)
```
...
myexp <- midar::calc_quantify_by_istd(myexp)
...
all available options for `qc_plot_runscatter()`.
...
specific QC criteria. The function `qc_apply_feature_filter()` lets you
apply such filters. Just press `TAB` after the open bracket of
`qc_apply_feature_filter()` to select, or press `F1` or run
`?qc_apply_feature_filter` to view, the available filter criteria. The
filter function may be applied multiple times, either overwriting or
amending previously set filters.
...
myexp <- qc_apply_feature_filter(
  ...
  istd.include = FALSE,
  response.curve.id = c(1,2),
  response.curve.summary = "mean",
  response.rsquare.min = 0.7,
  response.slope.min = 0.75,
  response.slope.max = 0.95,
  response.yintersect.max = 0.6,
...
We'll add the code for the extra QC analyses here before the workshop
starts.
