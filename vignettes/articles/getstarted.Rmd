...
myexp <- data_import_mrmkit(data = myexp, path = data_path, include_metadata = TRUE)
```
...
myexp <- data_exclude_analyses(myexp, analyses_exlude = c("Longit_batch6_51"), replace_existing  = TRUE)
...
myexp <- midar::qc_apply_feature_filter(myexp, min.intensity.median.spl = 200)
...
qc_plot_responsecurves(
  ...
  variable = "intensity", 
  filter_data = TRUE, 
  include_feature_filter = "^PC 3[0-6]", # using regular expressions    
...
```{r norm-effects}
myexp <- qc_apply_feature_filter(myexp, min.intensity.median.spl = 1000)
...
>
> Try changing `within_batch = FALSE` in the code chunk above with
...
> change back to `within_batch = TRUE` after the test.
...
`TAB` after the first open bracket of `qc_apply_feature_filter()`
function, or by to viewing the help page. by running
`?qc_apply_feature_filter` in the console. The filter function can be
applied multiple times, either overwriting or amending
(`replace_existing = FALSE`) previously set filters.
