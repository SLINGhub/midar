...
test_that("plot_runscatter with unknown qc_types", {
  ...
  expect_message(
    ...
    "QC types 'XYX, MyQC, BLK' not predefined in Midar and will be displayed in black with auto-assigned shapes",
    fixed = TRUE
  )
...
test_that("plot_runscatter handles filter and missing data", {
  ...
  mexp_filt <- midar::filter_features_qc(
    mexp,
    include_qualifier = FALSE,
    include_istd = FALSE,
...
test_that("plot_runscatter  error cap logscale", {
  ...
  expect_equal(length(p), 3)
  vdiffr::expect_doppelganger("plot_runscatter capoutlierlog", p)
})
...
test_that("plot_runscatter specific_page gried", {
  ...
  expect_equal(length(p), 1)
  vdiffr::expect_doppelganger("plot_runscatter page2grid", p[[1]])
})


test_that("plot_runscatter pdf multi4", {
  temp_pdf_path <- file.path(tempdir(), "midar_test_runscay2.pdf")
  mirai::daemons(4)
  p <- plot_runscatter(
    data = mexp,
    variable = "conc",
  ...
  expect_null(p)
  expect_true(file_exists(temp_pdf_path), info = "PDF file was not created.")
  expect_equal(as.character(fs::file_size(temp_pdf_path)), "125K")
  fs::file_delete(temp_pdf_path)
  mirai::daemons(0)
})
