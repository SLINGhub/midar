test_that("Orders samples according to parameter analysis_sequence", {
  ...
  mexp <- midar::import_masshunter(mexp, path =  testthat::test_path("22_Testdata_MHQuant_DefaultSampleInfo_RT-Areas-FWHM_notInSeq.csv"))
  ...
  mexp <- midar::import_masshunter(mexp, path =  testthat::test_path("23_Testdata_MHQuant_DefaultSampleInfo_RT-Areas-FWHM_notInSeq_notimestamp.csv"))
  mexp <- midar::import_metadata_msorganizer(mexp,
                                             path = testthat::test_path("MiDAR_Metadata_Template_191_20240226_MHQuant_S1P_V1.xlsm"),
...
test_that("Orders samples in daraset without timestamp field according to parameter analysis_sequence", {
  ...
  mexp <- midar::import_masshunter(mexp, path =  testthat::test_path("23_Testdata_MHQuant_DefaultSampleInfo_RT-Areas-FWHM_notInSeq_notimestamp.csv"))
  expect_error(midar::import_metadata_msorganizer(mexp,
                                             path = testthat::test_path("MiDAR_Metadata_Template_191_20240226_MHQuant_S1P_V1.xlsm"),
                                             analysis_sequence = "timestamp",
                                             excl_unannotated_analyses = FALSE),
...
test_that("Orders features by default according to order in metadata", {
  mexp <- midar::MidarExperiment()
  mexp <- midar::import_masshunter(mexp, path =  testthat::test_path("22_Testdata_MHQuant_DefaultSampleInfo_RT-Areas-FWHM_notInSeq-noalphafeat.csv"))
  mexp <- midar::import_metadata_msorganizer(mexp,
                                             path = testthat::test_path("MiDAR_Metadata_Template_191_20240226_MHQuant_S1P_V1.xlsm"),
                                             excl_unannotated_analyses = FALSE)
