  ...
  test_that("plot_normalization_qc() generates a plot", {
    ...
    expect_error(
      ...
      "`before_norm_var` and `after_norm_var` cannot be the same", fixed = TRUE
    )
    ...
    expect_error(
      ...
      "`after_norm_var` must be one of", fixed = TRUE
    )
    ...
    expect_error(
      ...
      "One or more specified `qc_types` are not present in the dataset", fixed = TRUE
    )
  ...
  test_that("plot_normalization_qc pre-flight checks for data state work", {
    ...
    expect_error(
      plot_normalization_qc(
        ...
        after_norm_var = "norm_intensity",
        qc_types = "BQC"),
      "Data has not yet been normalized", fixed = TRUE
    )
    ...
    expect_error(
      plot_normalization_qc(
        ...
        after_norm_var = "norm_intensity",
        plot_type = "diff"),
      "No QC metrics available yet", fixed = TRUE
    )
    ...
    expect_error(
      plot_normalization_qc(
        ...
        after_norm_var = "norm_intensity",
        plot_type = "diff"),
      "`before_norm_var` and `after_norm_var` must be supplied", fixed = TRUE
    )
    expect_error(
      plot_normalization_qc(
        mexp_no_metrics,
      ...
      plot_normalization_qc(
        ...
        before_norm_var = "intensity",
        after_norm_var = "norm_intensity"),
      "`plot_type` must be supplied ('scatter', 'diff', or 'ratio')", fixed = TRUE
    )
  })
  ...
  test_that("plot_normalization_qc constructs correct axis labels", {
    ...
    expect_equal(built_plot$plot$scales$get_scales("y")$name, "norm_intensity_cv - intensity_cv")
    expect_equal(built_plot$plot$scales$get_scales("x")$name, "Mean of intensity_cv and norm_intensity_cv")
    ...
    expect_equal(built_plot$plot$scales$get_scales("y")$name, "log2( norm_intensity_cv / intensity_cv )")
    ...
    built_plot <- ggplot_build(p_ratio)
    expect_equal(built_plot$plot$scales$get_scales("y")$name, "QC metric: norm_intensity_cv")
  })
  ...
  test_that("plot_qcmetrics_comparison plot looks as expected", {
    p <- plot_qcmetrics_comparison(
      ...
      plot_type = "scatter",
      share_y = FALSE,
      equality_line = FALSE,filter_data = FALSE,include_qualifier = FALSE,
      facet_by_class = TRUE
    )
    # Extract the plot's data (data frame used for the first layer)
    plot_data <- ggplot2::ggplot_build(p)$data[[1]]
  ...
  test_that("plot_qcmetrics_comparison plot looks as expected", {
    p <- plot_qcmetrics_comparison(
      ...
      plot_type = "scatter",
      share_y = FALSE,
      equality_line = FALSE,filter_data = FALSE,include_qualifier = TRUE,
      facet_by_class = TRUE
    )
    # Extract the plot's data (data frame used for the first layer)
    plot_data <- ggplot2::ggplot_build(p)$data[[1]]
  ...
  test_that("plot_qcmetrics_comparison data filter works", {
    p <- plot_qcmetrics_comparison(
      ...
      x_variable = "rt_median_bqc",
      y_variable = "norm_intensity_cv_bqc",
      plot_type = "scatter",filter_data = TRUE,
      share_y = FALSE,
      equality_line = FALSE,
      facet_by_class = TRUE
    )
  ...
  test_that("plot_qcmetrics_comparison plot looks as expected", {
    p <- plot_qcmetrics_comparison(
      ...
      share_y = FALSE, 
      threshold_value = c(NA, 10),
      facet_by_class = TRUE, log_scale = TRUE
    )
  ...
  test_that("plot_qcmetrics_comparison error handling works", {
    ...
    expect_error(
      ...
      "Log scale cannot be used with zero, negative, infinite, or NA axis limits", fixed = TRUE
    )
    ...
    expect_error(
        ...
        "`feature_class` to be defined in the metadata", fixed = TRUE
    )
  })
